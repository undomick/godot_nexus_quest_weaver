...
func initialize() -> void:
	...
	if not is_instance_valid(tree):
		push_error("QuestWeaver (SimpleKillAdapter): Could not get SceneTree access. Adapter will not function.")
		return
	...
	if not is_instance_valid(_kill_controller):
		if not _pending_game_state_connection:
			push_warning("QuestWeaver (SimpleKillAdapter): No 'kill_controller' in scene and no GameState registered. This adapter will not function.")
		return
	_finish_initialization(tree)

func _on_game_state_ready() -> void:
	if not _pending_game_state_connection: return
	var tree := Engine.get_main_loop() as SceneTree
	if not is_instance_valid(tree): return
	var services = tree.root.get_node_or_null("QuestWeaverServices")
	if not is_instance_valid(services): return
	...
	_kill_controller = services.get_game_state()
	if is_instance_valid(_kill_controller):
		_finish_initialization(tree)

func _finish_initialization(tree: SceneTree) -> void:
	if not is_instance_valid(_kill_controller): return
	if _kill_controller.has_signal("kill_count_changed") and not _kill_controller.is_connected("kill_count_changed", Callable(self, "_on_kill_count_changed")):
...
func count_kill(enemy_id: String) -> int:
	...
	if _kill_controller != null and _kill_controller.has_method("count_kill"):
		return _kill_controller.count_kill(enemy_id)
	return 0
