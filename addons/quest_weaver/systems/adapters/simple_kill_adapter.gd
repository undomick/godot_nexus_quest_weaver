...
func initialize() -> void:
	...
	if not is_instance_valid(_kill_controller):
		if not _pending_game_state_connection:
			push_warning("QuestWeaver (SimpleKillAdapter): No node in group 'kill_controller' and no QuestWeaverGameState registered. Add one for KILL objectives to work.")
		return
...
func count_kill(enemy_id: StringName) -> int:
	if not _is_initialized_successfully or not is_instance_valid(_kill_controller): return 0
