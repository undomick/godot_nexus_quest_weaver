...
func _init(p_event_node: EventNodeResource, p_entry_to_remove: EventNodeResource.PayloadEntry, p_index: int = -1):
	self._event_node = p_event_node
	self._entry_to_remove = p_entry_to_remove
	self._original_index = p_index if p_index >= 0 else (_event_node.payload_entries.find(_entry_to_remove) if is_instance_valid(p_entry_to_remove) else -1)
