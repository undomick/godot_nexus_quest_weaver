...
enum QuestAction {
	...
	MARK_AVAILABLE,      # Marks quest as AVAILABLE (e.g. for Quest Board, quest chains)
	MOVE_TO_CUSTOM_POOL  # Moves quest to a custom pool (e.g. daily, seasonal)
}
...
func _validate(context: Dictionary) -> Array[ValidationResult]:
	...
	if action == QuestAction.MOVE_TO_CUSTOM_POOL and custom_pool_id.is_empty():
		results.append(ValidationResult.new(ValidationResult.Severity.ERROR, "Quest Node: Custom Pool ID is not set for Move to Custom Pool action.", id))
		...
		if not pool_ids.has(custom_pool_id):
			results.append(ValidationResult.new(ValidationResult.Severity.WARNING, "Quest Node: Custom Pool '%s' not found in QuestWeaverSettings.additional_pool_scripts." % custom_pool_id, id))
	return results
