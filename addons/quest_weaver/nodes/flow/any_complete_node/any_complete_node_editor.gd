...
class_name AnyCompleteNodeEditor
...
func set_node_data(node_data: GraphNodeResource) -> void:
	...
	if not node_data is AnyCompleteNodeResource:
...
func _on_keep_listening_toggled(pressed: bool) -> void:
	if _is_setting_up:
		return
	if is_instance_valid(edited_node_data) and edited_node_data is AnyCompleteNodeResource:
